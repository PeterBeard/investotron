...
function RetirementPensionAccountSimulation({ simulationID, simulationTypeID, dispatchSimulationUpdate }) {
    ...
    const [ employerMatch, setEmployerMatch ] = useState(3_000);
    const [ employerMatchRatio, setEmployerMatchRatio ] = useState(100);
    ...
    function updateSimulationState(newState: PartialSimulationState) {
        ...
        function updateState(previousState: SimulationState, currentYear: number): SimulationState {
            ...
            let annualDeposit = (currentYear < previousState.withdrawalStartYear ? depositCount * previousState.deposit : 0);
            const totalWithdrawals = (currentYear >= previousState.withdrawalStartYear ? previousState.withdrawal : 0);

            if (previousState.hasEmployerMatch) {
                annualDeposit += Math.min(
                    (previousState.employerMatchRatio / 100.0) * annualDeposit,
                    previousState.employerMatch
                );
            }
            const newBalance = previousState.balance * (1 + previousState.interestRate / 100.0) + annualDeposit - totalWithdrawals;
    ...
    return (
        <Simulation
            ...
            <Stack spacing={1} direction="row">
                <Tooltip title="If checked, add an employer match to each deposit">
                    <FormGroup>
                        <FormControlLabel control={<Checkbox
                            name="has-employer-match"
                            size="small"
                            checked={hasEmployerMatch}
                            onChange={handleHasEmployerMatchChange} />}
                            label="Employer match?" />
                    </FormGroup>
                </Tooltip>
                <Tooltip title="Maximum amount of matching funds deposited by an employer each year. Matching funds are added to each deposit up to this maximum.">
                    <TextField
                        label="Maximum match ($)"
                        name="employer-match"
                        size="small"
                        value={employerMatch}
                        onChange={handleEmployerMatchChange} />
                </Tooltip>
                <Tooltip title="Percentage of each deposit the employer will match (up to the maximum)">
                    <TextField
                        label="Match ratio (%)"
                        name="employer-match-ratio"
                        size="small"
                        value={employerMatchRatio}
                        onChange={handleEmployerMatchRatioChange} />
                </Tooltip>
            </Stack>
        </Simulation>
    );
...
export { RetirementPensionAccountSimulation };
